...
class Amasty_Shopby_Block_Catalog_Layer_Filter_Attribute extends Mage_Catalog_Block_Layer_Filter_Attribute
{
    ...
    public function getIsSelected($item){
		$filter = $this->getRequestValue(); 
		$selectedValues = $this->getRequest()->getParam($filter);
		$selectedValues = explode(',',$selectedValues);
		$currItem = $item->getOptionId();
		
		if(in_array($currItem, $selectedValues)){
			return true;
		}else{
			return false;
		}
    ...
    public function getItemsAsArray()
    {
        ...
        foreach (parent::getItems() as $itemObject){
			...
			//print_r(get_class_methods($itemObject));
            $item['css'] = 'amshopby-attr';
             if ($this->getIsSelected($itemObject)){
                $item['css'] .= '-selected';
