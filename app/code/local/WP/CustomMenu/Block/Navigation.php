...
class WP_CustomMenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawCustomMenuItem($category, $level = 0, $last = false)
    {
        ...
        if ($drawPopup) {
            $html[0][] = '<div id="menu' . $id . '" class="menu' . $active . '" onmouseover="wpShowMenuPopup(this, event, \'popup' . $id . '\');" onmouseout="wpHideMenuPopup(this, event, \'popup' . $id . '\', \'menu' . $id . '\')">';
        } else {
            $html[0][] = '<div id="menu' . $id . '" class="menu' . $active . '">';
        }
        // --- Top Menu Item ---
        $html[0][] = '<div class="parentMenu">';
        if ($level == 0 && $drawPopup) {
            if($category['name'] !="Underwear"){
                $html[0][] = '<a href="javascript:void(0);" rel="'.$this->getCategoryUrl($category).'">';
            }
            else {
                $html[0][] = '<a href="#" class="unbind">';    
            }
        } else {
            $html[0][] = '<a href="'.$this->getCategoryUrl($category).'">';
        }
        $name = $this->escapeHtml($category->getName());
        if (Mage::getStoreConfig('custom_menu/general/non_breaking_space')) {
            $name = str_replace(' ', '&nbsp;', $name);
        }
        $html[0][] = '<span>' . $name . '</span>';
        $html[0][] = '</a>';
        $html[0][] = '</div>';
        $html[0][] = '</div>';
        // --- Add Popup block (hidden) ---
        if ($drawPopup) {
            ...
            $html[1][] = '<div id="popup' . $id . '" class="wp-custom-menu-popup " onmouseout="wpHideMenuPopup(this, event, \'popup' . $id . '\', \'menu' . $id . '\')" onmouseover="wpPopupOver(this, event, \'popup' . $id . '\', \'menu' . $id . '\')">';
            // $html[1][] = '<div id="popup'.$id.'">';

            $html[1][] = '<div class="main_category_block">';
            // --- draw Sub Categories ---
            if (count($activeChildren)) {
                $columns = (int)Mage::getStoreConfig('custom_menu/columns/count');

                $html[1][] = '<div class="block1">';
                $html[1][] = $this->drawColumns($activeChildren, $columns);
                $html[1][] = '<div class="clearBoth"></div>';
                $html[1][] = '</div>';
            }
            // --- draw Custom User Block ---
            if ($blockHtml) {
                $html[1][] = '<div class="right-slider-wrapper">';
                $html[1][] = '<!--div class="slider_title">This week FC loves</div-->';
                $html[1][] = '<div id="' . $blockId . '" class="block2 bxslider-menu">';
                $html[1][] = $blockHtml;
                $html[1][] = '</div>';
                $html[1][] = '</div>';
            }
            ...
            $html[1][] = '</div>';
            // $html[1][] = '<div class="delivery_message">
            //               <a href="#">Free Uk delivery when you spend £75 + Free returns*</a>
            //               </div>';
            $html[1][] = '</div>';
        }
        // echo "<textarea>";print_r($html);echo "</textarea>";
        // $html = implode("\n", $html);
        return $html;
    }
    public function makeStringMenu($html) {
    ...
    public function drawMenuItem($children, $level = 1)
    {
        ...
        foreach ($children as $child)
        {
            if (is_object($child) && $child->getIsActive())
            {
                ...
                if (Mage::getStoreConfig('custom_menu/general/non_breaking_space'))
                    $name = str_replace(' ', '&nbsp;', $name);
                $thumbnail = Mage::getModel('catalog/category')->load($child->getId())->getThumbnail();
                if($thumbnail){
                    $pathThumbnail = $this->getUrl().'media/catalog/category/'.$thumbnail;
                    //var_dump($pathThumbnail);exit;
                    $html.= '<img src="'.$pathThumbnail.'"/>';
                }
                 if($name=="SALE"){ 
                    $saleclass = "salemenu";
                 }

                $html.= '<a class="itemMenuName level' . $level . $active . ' '.$saleclass.'" href="' . $this->getCategoryUrl($child) . '"><span>' . $name . '</span></a>';
                $activeChildren = $this->_getActiveChildren($child, $level);
