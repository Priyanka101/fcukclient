...
class WP_CustomMenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawCustomMenuItem($category, $level = 0, $last = false)
    {
		...
		$query = "SELECT * FROM landingpages where catname='womanmenuproduct'";
		$result=$read->query($query);
		$landingpage = $result->fetchAll();
		//print_r($landingpage[0]['content']);
		//$i=0;
		if($id==3 ||$id==4){
		if($id!=$prev){
		$prev=$id;
		$tag=$landingpage[$i]['content'];
		$_product = Mage::getModel('catalog/product')->loadByAttribute('sku', $tag);
		$productid=$_product->getData('entity_id');
		//print_r($_product->getData());																
		//print_r($_product->getData('image'));																
	...
	///	echo '<pre>';
        ...
        if ($drawPopup) {
            ...
            if (count($activeChildren)) {
				...
				if($image_url!=''){
            $html[]='<div class="column last odd">';
			$html[]='<img class="'.$image_name.'" width="200px" src="'.$image_url.'"/>';
			$html[]='<a href="'.$productlink.'">'.$productname.'</a>';
			$html[]='</div>';
			 
			$html[]='<div class="column last even">';
			$html[]='<img class="'.$image_name.'" width="200px" src="'.$image_url.'"/>';
			$html[]='<a href="'.$productlink.'">'.$productname.'</a>';
			$html[]='</div>'; 
}
