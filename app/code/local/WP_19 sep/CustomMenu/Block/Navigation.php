...
class WP_CustomMenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawCustomMenuItem($category, $level = 0, $last = false)
    {
        ...
        $html = array();
		$id = $category->getId();
		$cats = Mage::getModel('catalog/category')->load($id);
		//print_r($cats->getData());
		$baseurl=$this->getBaseUrl().'media/catalog/category/';
		$image_url1 = $baseurl.''.($cats->getData('image1'));
		$image_url11 = $cats->getData('image1');
		$image_url2 = $baseurl.''.($cats->getData('image2'));
		$image_url22 = $cats->getData('image2');
		$url1 = $cats->getData('url1');
		$url2 = $cats->getData('url2');
	
	/* 	$image_url = $cats->getImageUrl(); */
		$image_name1 = $cats->getData('name1');
		$image_name2 = $cats->getData('name2');
		///	echo '<pre>';
        //	print_r($cats->getData());
        ...
        $active = ''; if ($this->isCategoryActive($category)) $active = ' act newact';
        ...
        if ($drawPopup) {
            ...
            if (count($activeChildren)) {
				...
				if($image_url11!=''){
            $html[]='<div class="column last odd menuimage1">';
			$html[]='<a href="'.$url1.'">';
			$html[]='<img class="'.$image_name1.'" src="'.$image_url1.'" style=""/>';
			$html[]='</a>';
			$html[]='<a href="'.$url1.'"  class="menuimage" >'.$image_name1.'</a>';
			$html[]='</div>';
			}
			if($image_url22!=''){
			$html[]='<div class="column last even menuimage2">';
			$html[]='<a href="'.$url2.'">';
			$html[]='<img class="'.$image_name2.'" src="'.$image_url2.'" style=""/>';
			$html[]='</a>';
			$html[]='<a href="'.$url2.'" class="menuimage">'.$image_name2.'</a>';
			$html[]='</div>'; 
			}
    ...
    public function drawMenuItem($children, $level = 1)
    {
        ...
        //$keyCurrent = $this->getCategory()->getId();
		//$cats = Mage::getModel('catalog/category')->load($keyCurrent);
		//echo '<pre>';print_r($cats->getData());
        ...
        foreach ($children as $child)
        {
            ...
            if (is_object($child) && $child->getIsActive())
            {
                ...
                $id = $this->escapeHtml($child->getId());
				
				$parentId=Mage::getModel('catalog/category')->load($id)->getParentId();
				$parentparentId=Mage::getModel('catalog/category')->load($parentId)->getParentId();
				$catagory_model = Mage::getModel('catalog/category');
				$categories = $catagory_model->load($parentparentId); // where $id will be the known category id
				$categoryname=$categories->getName(); //get category name     
				$catwithoutspace=str_replace(' ','',$name);
				$finalcat=strtolower($categoryname.''.$catwithoutspace);          
				
				//$categories->getName();
                if (Mage::getStoreConfig('custom_menu/general/non_breaking_space'))
                    $name = str_replace(' ', '&nbsp;', $name);
					
                $html.= '<a class="itemMenuName categoryname  '.$finalcat.'  level' . $level . $active . '" href="' . $this->getCategoryUrl($child) . '"><span>' . $name . '</span></a>';
