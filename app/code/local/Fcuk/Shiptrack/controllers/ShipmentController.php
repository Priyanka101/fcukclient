<?php
include_once('Mage/Adminhtml/controllers/Sales/Order/ShipmentController');
class Fcuk_Shiptrack_ShipmentController extends Mage_Adminhtml_Sales_Order_Shipment
{
    /**
	...
	//	$tracking = Mage::getModel('shiptrack/shipment')->ondeleteupdate($trackId);
		Mage::getModel("shiptrack/shipment")->onDeleteUpdate($track->getId());
        if ($track->getId()) {
            try {
                if ($this->_initShipment()) {
					//echo 456;
					//$tracking->ondeleteupdate($trackId);
					//$tracking = Mage::helper('shiptrack/shipment')->onDeleteUpdate($track->getId());
					//echo $tracking;
					$track->delete();
					$this->loadLayout();
