...
class Fcuk_All_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getColorOptions($_product)
	{
		...
		foreach ($products as $product) {
			...
			$size_salable = array('size' => $currSize, 'is_salable' => $currIsSalable ,'color_code'=> $colorCode  ,'size_code'=> $sizeCode,'color_id'=>$color_id);
		...
		//asort($finalSize);
		//echo '<pre>';print_r($finalSize);exit;
		...
		foreach($finalGrid as $option=>$val){
			...
			$img_dir = Mage::getBaseDir('media').'amconf/images/'.$color_id.'.jpg';
			$img_src = '';
			...
			if(file_exists($img_dir)){
				$img_src = Mage::getBaseUrl('media').'amconf/images/'.$color_id.'.jpg';
			}
			//echo $img_src;exit;
			$html .= '<tr>';
			$html .= '<td style="width:120px;text-align:center" >
			<div style="width:34px;float:left; height:34px; background:'.$option.'; margin-right:5px;">';
