...
class Fcuk_All_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getColorOptions($_product)
	{
		...
		//print_r($val);
			...
			$swatch=strtolower($swatchnew[0]).'_'.strtolower(str_replace('/','-',$color_name_new));
			//echo $swatch;exit; 
...
/* 	public function getPreviousProduct(){
	...
	/*Added for ss14 theme*/
	public function getBannerCategories($categoryId){
		$model = Mage::getModel("catalog/category");
		$category= $model->load($categoryId);
		$data['url']=$category->getUrl();
		$child_category=$category->getChildrenCategories();
		foreach ($child_category as $categories) {
			$cats = $categories->getChildrenCategories();
			foreach($cats as $categ){
				$cat = $model->load($categ->getId());
				if($cat->getShowOnBanner()){
					$show[] = array(0=>$category->getName(), 1=>$category->getUrl()); 
				}
			}
	...
	public function subcategory(){
		$current_catid = Mage::getModel('catalog/layer')->getCurrentCategory()->getId(); 
		$cat = Mage::getModel('catalog/category')->load($current_catid); 
    ...
    public function getSubcategory($categoryId){
	    ...
	    foreach($subcategories as $subcategory){
			$_category = $model->load($subcategory);
			if($_category->getIsActive()){
				$data[] = array(0=>$_category->getUrl(),1=>$_category->getName());
	    	}
