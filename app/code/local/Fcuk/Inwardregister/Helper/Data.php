...
public function setEnable($productId, $newQty){
		...
		//print_r($productStatus->getData());
		//print_r($parentProductStatus->getComingsoon());exit;
		/*set coming soon to NO after qty uploaded even if product does not have images*/
		if($parentProductStatus->getComingsoon()){
			$productStatus->setComingsoon(0)->save();
			$parentProductStatus->setComingsoon(0)->save();
		...
		if(count($total_images) > 0){
			...
			/*commented by shaily as the stock status will be same as uploaded.*/
