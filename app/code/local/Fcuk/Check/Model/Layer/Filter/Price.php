 ...
 */
class Fcuk_Check_Model_Layer_Filter_Price extends Mage_Catalog_Model_Layer_Filter_Price
{
    /**
	...
	$categoryModel = Mage::getModel('catalog/category')->load(Mage::registry('current_category')->getId()); // Replace with Id of Cat
...
$formattedFromPrice  = $store->formatPrice($lowestProductPrice);
        ...
        if ($toPrice === '') {
            return Mage::helper('catalog')->__('%s and above', str_replace(".00", "", $formattedFromPrice));
            ...
            return Mage::helper('catalog')->__('%s - %s', str_replace(".00", "", $formattedFromPrice), str_replace(".99", "", $store->formatPrice($toPrice)));
        }
    }
