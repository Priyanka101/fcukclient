     ...
     //    			->load();
            ...
            // This will give me the shipment IncrementId, but not the actual tracking information.
				...
				$HTML = '<table border="0">
						...
						<tr><td colspan="2" >&nbsp;</td></tr>
						<tr><td colspan="2" >&nbsp;</td></tr>
						<tr><td colspan="2" style="border:1px solid #999;" ><table cellpadding="3"><tr><td><strong>Brand Retail Pvt Ltd.</strong><br />C/o FedEx Express Transportation and Supply Chain Services (India) Private Limited<br />Next to NDR Warehousing,Survey No. 95 A , Hissa No. 5 , Mumbai Nasik Highway,<br/>
						...
						<tr>
							...
							<td colspan="2" style="border:1px solid #999;">
								<table>
									...
									<tr>
										<td style=" ">CIN No</td>
										<td style=" text-align:right;">U54131MH2005PTC157347</td>
									</tr>
									<tr>
										<td >&nbsp;</td>
										<td >&nbsp;</td>
								...
								<table width="100%" style=" border:1px solid #999;  width:215px;">
									...
									<tr style="text-align:center;">
										...
										foreach($order->getAllItems() as $item){
											...
											$netValuenew=$item->getData('price_incl_tax');  // = 75
											$netValue=($item->getData('base_row_total_incl_tax'))-($item->getDiscountAmount());  // = 75

											

											$itemName=$item->getName();  // = 75
											
											$productcode=$item->getSku();  // = 75
										
											$specialprice=$item->getOriginalPrice(); 
											
											$dis_val=$item->getDiscountAmount();
											
											$vat_tax=$item->getTaxAmount();
										
											$qty=$item->getQtyOrdered();
											
											$attr =  Mage::getModel('catalog/product')->loadByAttribute('sku',$item->getSku());
											...
											$mrpValue=$item->getData('custom_original_price');
											$grossValue=$mrpValue*$qty;
											$finaldiscounts=$dis_val+($mrpValue-$specialprice);
											if($specialprice!=$netValuenew)
											{
												$finaldiscounts=$dis_val;
											}
											...
											if(is_numeric($productcode)){
												...
												if($vat_tax=='0'){
													...
													$vat_tax= sprintf("%.2f", ($mrpValue * $vat)/(100+$vat));
												}
												
											}

											$total_val=$total_val+$netValue;
											...
											<tr style="text-align:center;">
												...
												<td>'.sprintf("%.2f", $finaldiscounts).'</td>
