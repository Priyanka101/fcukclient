		...
		if (!$this->hasData('strands_id')) {
			...
			if ($session->isLoggedIn()) {
				if (($email = $session->getCustomer()->getEmail()) != '') {
					$this->setStrandsId($session->getCustomer()->getEmail());
					Mage::getModel('core/cookie')->set('strandsSBS_P_UserId',$session->getCustomer()->getEmail(),true,'/',null,null,false);
				} else {
...
*/
	...
	public function writeCatalog($localpath,$localfile,$page = 0, $maxcount = 0) 
	{
        ...
        $totalItems = $collection->getSize();
        $this->writeIntoFile($scw,'product_count',(string)$totalItems,1);	    
