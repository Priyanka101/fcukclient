...
class KM_Shippingcarriers_Model_Quantium extends Mage_Shipping_Model_Carrier_Abstract implements Mage_Shipping_Model_Carrier_Interface {

    protected $_code = 'quantium';
    ...
    public function getAllowedMethods() {
        ...
        return array('quantium' => $this->getConfigData('title'));
    }
    ...
    public function getTracking($tracking_number) {
        ...
        $path = $this->getConfigData('gateway_url') . '?trackingno=' . $tracking_number;
		$ch = curl_init();
        ...
        if ($xmlobject) {
            ...
            $status = "";
            if (isset($xmlarray['StageHeader'])) {
                foreach ((array) $xmlarray['StageHeader'] as $k => $v) {
                    if ($k == 'CHECKPOINTDETAILS')
                        continue;
					if($k == 'SenderInformation'){
						if(empty($v)){
							continue;
							}
					...
					if($k == 'ReceiverContactNo'){
						if(empty($v)){
							continue;
							}
						
						}
					if($k == 'SpecialDeliveryInstruction'){
						if(empty($v)){
							continue;
							}
					...
					if($k == 'Details'){
					foreach($v as $newdetail){
					$status .= "<strong>" . $k . "</strong>";	
					$status .=	" : StageDetails: " . $newdetail['StageDetails'] . " , StatusUpdatedDate: " . $newdetail['StatusUpdatedDate'] . "UpdatedLocation: " . $newdetail['UpdatedLocation'] . " , Remarks: " . $newdetail['Remarks'] . " , <br/>";
					}
					continue;
					}
